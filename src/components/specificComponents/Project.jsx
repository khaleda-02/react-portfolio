...
function NonPreferdProject({ id, title, imgScr, link, github, data, skills}) {
  return (
    <>
      <div className='hidden lg:flex  w-[100%] md:w-[47%] relative  justify-center items-center group my-10 text-slate'>
        ...
        <div className='z-50 group absolute top-0 right-0 opacity-0 group-hover:opacity-[1] duration-500 w-full h-full flex items-end  justify-end gap-7 p-6'>
          <a href={github} target="_blank" rel="noreferrer" className=' animate-bounce h-8 '><RiGithubLine size={50} /></a>
          <a href={link} target="_blank" rel="noreferrer" className=' animate-bounce h-8 '><TiArrowForwardOutline size={50} /></a>
        </div>
      </div>
      {mobileStructure({ id, title, imgScr, link, github, data, skills })}
    </>
  )
}
...
const mobileStructure = ({ id, title, imgScr, link, github, data, skills }) => (
...
{/* <div className=' w-[100%] min-h-[500px]   relative  justify-cneter items-center group my-10 rounded-sm'>
<div className={`absolute min-w-full min-h-full top-0 left-0 bg-light-navy/90 rounded-sm py-2 px-5 flex flex-col items-start justify-center `}>
  <div className='cairo font-extrabold capitalize text-4xl tracking-wider text-green'>{title}</div>
  <p className='text-white text-lg  rounded-sm tracking-normal leading-8 my-3 '>{data}</p>
  <div className={`flex  items-center justify-start`}>{skills.map(skill => {
